  ...
  <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <h1 class="text-gray-900 text-5xl"><?php $patient_name;?></h1>
        <?php
        if (isset($_POST['submit'])) {
          ...
          if ($result2 == TRUE) {
            echo '<h3>Appointment succesfully completed</h3>';
          } else {
          ...
          <?php $sql = "SELECT a.date, CONCAT(u.fname, ' ', u.lname) as dr_name FROM appointment a, user u WHERE a.patient_id='$patient_id'
            ...
            if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                ...
                //checks if its today
                if ($next_date == $today) {
                    ?><form class="mt-8 space-y-6" name='complete' action='' method='POST'>
